...
variable "task_role_ooo_config" {
  ...
  default     = null
  description = <<EOF
  A list of objects that contains the settings for the Out-of-the-box task role, compatible
with ECS to be used as a task role (task_role_arn if it's used in a task definition)
The current attributes that are supported are:
- name: The unique identifier of this resource.
- role_name: The name of the role. If it's not passed, it'll take the name passed
  in the "name" attribute.
...
variable "task_role_permissions_config" {
  type = list(object({
    role_name                      = string
    policy_name                    = string
    resources                      = optional(list(string), ["*"])
  ...
  description = <<EOF
  A list of objects that contains the settings for the permissions that will be attached
to the task role, compatible with ECS (Task or ECS service).
The current attributes that are supported are:
- role_name: The name of the role. If it's not passed, it'll take the name passed
  in the "name" attribute.
- resources: The ARN of the resource that will be attached to the role.
